...
class CanCorr():
    ...
    def canoniCorr(self):
        ...
        #有意水準を求める
        sig = sp.special.chdtri(xc*yc,0.05)
    ...
    def chisq(self, p, kk):
        ...
        #print math.pow(0.5,k/2)
        #print math.pow(x,k/2-1)
        #print math.exp(-x/2)
        #print "gamma:"+str(math.gamma(k/2))
        #f = (math.pow(0.5,k/2)/math.gamma(k/2))*math.pow(x,k/2-1)*math.exp(-x/2)
        f = integrate.quad(lambda x, k: (math.pow(x,k/2-1) * math.exp(-x/2))/(math.pow(2,k/2)*math.gamma(k/2)), p, np.inf, args=kk)
        #f = SY.integrate((math.pow(x,k/2-1) * math.exp(-x/2))/(math.pow(2,k/2)*math.gamma(k/2)),(x,p,oo))
        return f
